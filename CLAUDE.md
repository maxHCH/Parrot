...
popup.*          每日開口進度（進度環、streak、週曆、凍結）+ Quest 記分板（LV、今日 XP、🔥 streak、生詞數）
...
- **生詞高亮**：句子清單裡生詞本的字標色（🟠 新字／🟡 複習中／🟢 interval ≥ 21 天）。字表來自 background 的 `vocabCache`（登入後讀 `vocabularies` 的 SRS 欄位，TTL 10 分鐘；沒登入不高亮。存新字會直接補進快取，面板靠 `storage.onChanged` 即時重標）。比對用 lookaround 不用 `\b`（生詞本有 `ref()` 這種符號結尾的詞）。先聽再看模式會藏掉色塊防洩題。
- **練習完成度**：跟讀過（完成至少一輪循環）的句子打勾，面板顯示「跟讀 12/38」，存 `done:{videoId}`。key 是句子起始時間 ×10 取整，不是 index（換字幕軌 index 會位移）。載入字幕後自動捲到上次練到的句子；練完最後一個新句子跳「整支練完 🎉」（重開已練完的影片不會跳）。
...
- **生詞本**：小卡的「加入生詞本」→ upsert 進 `vocabularies` 表（Parrot Nest 的 SRS 直接吃得到）。去重靠先查再 PATCH/POST（unique index 是 `lower(word)` expression index，PostgREST 的 `on_conflict` 吃不到，只當最後防線）；只覆蓋內容欄位，不動 `due`/`ease` 等排程欄位。migration 002 之後 RLS 只認 `auth.uid()`，**存字、字幕高亮都要登入**（沒登入 saveWord 回 `needLogin`，小卡提示去選項頁登入）。
...
- schema 的唯一真相來源：`~/Projects/vocab-app/supabase/migrations/`（001 記分層、002 vocabularies 收歸帳號＋成就補充）。
...
- [x] 字幕生詞高亮（zeroStudy 式）：SRS 狀態三色標記 + 面板生詞計數 legend
- [x] 影片練習完成度：每句打勾、面板顯示 12/38、續練自動捲動（`done:{videoId}`）
- [x] popup 記分板：LV + 升級進度條 + quest streak + 生詞數（`quest-today` 一次回傳 my_stats／今日 XP／vocabularies count）
- [ ] 聽寫計分：blur 模式打字比對 diff，全對給 bonus XP（新 source `dictation`）
- [ ] 週報推播：每週一查上週 `v_daily_summary` → 通知 → 點開 Nest /stats
