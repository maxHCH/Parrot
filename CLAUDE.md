...
- **生詞高亮**：句子清單裡生詞本的字標色（🟠 新字／🟡 複習中／🟢 interval ≥ 21 天）。字表來自 background 的 `vocabCache`（anon 讀 `vocabularies` 的 SRS 欄位，TTL 10 分鐘；存新字會直接補進快取，面板靠 `storage.onChanged` 即時重標）。比對用 lookaround 不用 `\b`（生詞本有 `ref()` 這種符號結尾的詞）。先聽再看模式會藏掉色塊防洩題。
